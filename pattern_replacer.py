...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        examples.setPlainText(
            ...
            "\n"
            "- Count-based sum mode (generates 'count' totals between lower and upper):\n"
            ...
            "  <sum_inc count=5 lower=0 upper=1 dist=random min_mult=0.25 max_mult=1.5 places=3> a:0,b:0 </sum_inc>\n"
            "\n"
            "- dist = uniform | normal_center | normal_start | normal_end | random\n"
            "- random: per-row, each variable is randomized within [min_mult*base, max_mult*base],\n"
            "  where base = total/nslots, while keeping the row sum equal to the target total.\n"
            "- places (aliases: decimals, precision, dp) forces decimal places; otherwise auto.\n"
            "\n"
