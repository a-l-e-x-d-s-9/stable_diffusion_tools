...
class ImageDropWidget(QWidget):
    def __init__(self, args, parent=None):
        ...
        # Create and set up the Load Overlay button
        self.load_overlay_btn = QPushButton("Load Overlay", self)
        self.load_overlay_btn.clicked.connect(self.load_overlay_image)
        self.overlay_buttons_layout.addWidget(self.load_overlay_btn)

        # Create and set up the Add button
        self.add_overlay_btn = QPushButton("Add", self)
        self.overlay_buttons_layout.addWidget(self.add_overlay_btn)
        self.add_overlay_btn.clicked.connect(self.add_overlay_to_preview)

        # Create and set up the Save button
        self.save_overlay_btn = QPushButton("Save", self)
        self.overlay_buttons_layout.addWidget(self.save_overlay_btn)
        self.save_overlay_btn.clicked.connect(self.save_with_overlays)

        # Set alignment for the button layout to right
        self.overlay_buttons_layout.setAlignment(Qt.AlignRight)

        # Create and set up the overlay image label
        self.overlay_image_label = QLabel(self)
        self.overlay_image_label.setFixedSize(100, 100)
        self.overlay_image_label.setAlignment(Qt.AlignLeft)
        self.overlay_image_label.setStyleSheet("border: 1px solid black;")

        # Create a horizontal layout to group the overlay image and buttons
        self.overlay_ui_layout = QHBoxLayout()

        # Add the overlay image label and the button layout to the horizontal layout
        self.overlay_ui_layout.addWidget(self.overlay_image_label)
    ...
    def save_with_overlays(self):
        # # Create a QPixmap to hold the final composited image
        # final_pixmap = QPixmap(self.base_image_pixmap.size())
        # final_pixmap.fill(Qt.transparent)  # Start with a transparent background
        #
        # # Create a QPainter to draw onto the final_pixmap
        # painter = QPainter(final_pixmap)
        # painter.drawPixmap(0, 0, self.base_image_pixmap)  # Draw the base image first
        #
        # # Grab the current visual state of the preview label with all overlays
        # overlays_capture = overlays_capture = self.preview_label.grab()
        # painter.drawPixmap(0, 0, overlays_capture)  # Draw the overlays on top
        #
        # painter.end()  # Finish the painting
        #
        # # Save the final composited image to disk
        # save_path, _ = QFileDialog.getSaveFileName(self, "Save Image", "", "Images (*.png *.xpm *.jpg);;All Files (*)")
        # if save_path:
        ...
        # painter.drawPixmap(0, 0, self.base_image_pixmap)  # Draw the base image first
        #
        # # Calculate the scale factor between the original image and the previewed image
        # scale_factor_width = self.base_image_pixmap.width() / self.preview_label.width()
        # scale_factor_height = self.base_image_pixmap.height() / self.preview_label.height()
        #
        # # Find all OverlayLabel children of preview_label
        # for overlay in self.preview_label.findChildren(OverlayLabel):
        #     # Calculate the position and size of the overlay on the original image using the scale factors
        #     overlay_x = int(overlay.x() * scale_factor_width)
        #     overlay_y = int(overlay.y() * scale_factor_height)
        #     overlay_width = int(overlay.width() * scale_factor_width)
        #     overlay_height = int(overlay.height() * scale_factor_height)
        #
        #     # Scale the overlay pixmap to the calculated size
        #     scaled_overlay_pixmap = overlay.pixmap().scaled(overlay_width, overlay_height, Qt.KeepAspectRatio,
        #                                                     Qt.SmoothTransformation)
        #
        #     # Draw the scaled and positioned overlay onto the final_pixmap
        ...
        if not self.base_image_pixmap:
            QMessageBox.warning(self, "Error", "No base image loaded!")
            return
        ...
        for overlay in self.findChildren(OverlayLabel):
            # Calculate the position and size of the overlay on the original image using the scale factors
            overlay_x = int(overlay.x() * scale_factor_width)
            overlay_y = int(overlay.y() * scale_factor_height)
            overlay_width = int(overlay.width() * scale_factor_width)
            ...
            # Scale the overlay pixmap to its proportional size on the original image
            scaled_overlay_pixmap = overlay.pixmap().scaled(overlay_width, overlay_height, Qt.KeepAspectRatio,
                                                            Qt.SmoothTransformation)

            # Draw the scaled overlay pixmap onto the final image
            painter.drawPixmap(overlay_x, overlay_y, scaled_overlay_pixmap)
    ...
    def add_overlay_to_preview(self):
        if self.overlay_image_pixmap:
            # Calculate 60% of the preview label's width
            target_width = int(self.preview_label.width() * 0.6)

            # Scale the overlay pixmap to the target width while maintaining aspect ratio
            scaled_pixmap = self.overlay_image_pixmap.scaledToWidth(target_width, Qt.SmoothTransformation)

            # Store the original overlay image pixmap
            self.original_overlay_pixmap = self.overlay_image_pixmap

            # Create and show the OverlayLabel
            self.overlay_label = OverlayLabel(self, self.preview_label)
            self.overlay_label.setPixmap(scaled_pixmap)
            self.overlay_label.resize(scaled_pixmap.size())  # Adjust the label size to fit the pixmap
    ...
    def load_overlay_image(self):
        ...
        file_name, _ = QFileDialog.getOpenFileName(self, "Load Overlay Image", "",
                                                   "Images (*.png *.xpm *.jpg);;All Files (*)", options=options)
        if file_name:
            self.overlay_image_path = file_name
            self.overlay_image_pixmap = QPixmap(file_name)
            self.overlay_image_label.setPixmap(
                self.overlay_image_pixmap.scaled(self.overlay_image_label.size(), Qt.KeepAspectRatio,
                                                 Qt.SmoothTransformation))
